...
import { getConnection, getPayer } from "../accounts/connection";
...
async function createTokenForLiquidity(): Promise<TokenCreationResult> {
  try {
    ...
    const tokenDetails: TokenDetails = {
      ...
      metadataUri,
      totalSupply: `${tokenSupply / Math.pow(10, tokenDecimals)} ${
        metadata.symbol
      }`,
    ...
    console.log(
      `Total Supply: ${tokenSupply / Math.pow(10, tokenDecimals)} ${
        metadata.symbol
      }`
    );
